...
class Budget
{
    ...
    public function cogsTarget($target = null, $category = null)
    {
        ...
        if( ! is_null($target)) // If something is in target
        {
            ...
            if( ! is_null($category)){ //If something is in category
                ...
                $this->cogsCategoryTarget(); //Call the method with a null category to assign this cogs value to all categories
            }
        } else {
    ...
    public function cogsCategoryTarget($category = null)
    {
        ...
        if(isset($this->categoryTarget[$category])) { // Checks to see if the category has been set
            $newTarget = $this->categoryTarget[$category]; // If it is set, assigns new target to the value in that index
            $newTarget = number_format((float)$newTarget, 1, '.', '');

            $this->byCategory($category); //Call the byCategory function in order to calculate budget
        } else { //If the specified category index isnt found, just assign new index to the default cogs value
            $newTarget = $this->cogs_target;
            $this->categoryTarget[$category] = $newTarget;
            $newTarget = number_format((float)$newTarget, 1, '.', '');
